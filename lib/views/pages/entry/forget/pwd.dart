          ...
          children: <Widget>[
            ...
            Input(
              ...
              onInput: (String val) => setState(() => _canSubmit =
                  (_confirmController.text.length > 5 && val.length > 5)),
              onComplete: () {
                if (_canSubmit) {
                  _submit();
                } else
    ...
    if (_canSubmit) {
      if (_controller.text == _confirmController.text) {
        if (_isClick) {
          ...
          $Http(lang: $SP.getString('lang', def: 'en-us')).post(
            API.forget['step_3'],
            data: {'uid': $SP.getString('uid'), 'password': _controller.text},
          ).then((res) {
            _isClick = true;
            ...
            if (res.code == 0)
              showDialog(
                ...
                barrierDismissible: false,
                builder: (_) => AppDialog(
                  text: I18n.of(context).$t('forgetSuccess'),
                  complete: () {
                    $SP.remove('uid');
                    Navigator.of(context).pop();
                  },
