...
class Invaild extends StatefulWidget {
  @override
  _InvaildState createState() => _InvaildState();
}

class _InvaildState extends State<Invaild> {
  int _page = 1;
  Future _future;
  ...
  Future<List<NodeUserModel>> _get(bool refresh) async {
    ...
    ).get(API.node['list'], params: {'page': _page, 'is_valid': 0});
    if (_res.code == 0)
      refresh
            ...
            Column(
              ...
              children: <Widget>[
                Container(
                  margin: EdgeInsets.only(bottom: 5),
                  child:
                      Text(Levels.v(model.level), style: AppText.f14_w4_A3ACBB),
                ),
